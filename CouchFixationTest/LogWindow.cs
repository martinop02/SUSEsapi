...
namespace CouchFixationTest
{
    ...
    ///
    /// The work runs synchronously on the ESAPI main thread (as ESAPI requires for model
    /// access); lines are collected and shown in a scrollable, read-only window at the end.
    /// A separate <see cref="LogWindow"/> is intentionally simple so iteration stays fast —
    /// swap in the richer PalliativeAutoPlan LogWindow later if live progress is needed.
    /// </summary>
