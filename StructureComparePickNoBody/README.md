...
This is the **no-BODY** build: the structure named `BODY` (case-insensitive) is excluded from
matching, so the external/body contour is never compared. It is otherwise identical to
`StructureComparePick`, and its Debug build deploys to the **same** `StructureComparePick` folder
(the two DLLs, `StructureComparePick.esapi.dll` and `StructureComparePickNoBody.esapi.dll`,
coexist there).
...
1. Build in Visual Studio (net48, x64) → `StructureComparePickNoBody.esapi.dll`. **No NuGet dependencies.**
